...
fn main() {
    ...
    builder.br(cond, vec![argc, argv]);
    builder.set_block(cond);
    ...
    let _ = builder.call(Const::Addr(puts), arg);
    ...
    let asm = codegen::run(&module, codegen::Backend::Clif);

    std::process::Command::new("cc").arg(asm.path()).arg("-o").arg("test").status().unwrap();
}
