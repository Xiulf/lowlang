...
pub fn layout_of(ty: &Ty, target: &Triple) -> TyLayout {
    ...
    let mut layout = match &ty.kind {
        | Type::Int(1, _) => scalar(Primitive::Bool),
        | Type::Int(8, sign) => scalar(Primitive::Int(Integer::I8, *sign)),
        | Type::Int(16, sign) => scalar(Primitive::Int(Integer::I16, *sign)),
        | Type::Int(32, sign) => scalar(Primitive::Int(Integer::I32, *sign)),
        | Type::Int(64, sign) => scalar(Primitive::Int(Integer::I64, *sign)),
        | Type::Int(128, sign) => scalar(Primitive::Int(Integer::I128, *sign)),
        | Type::Int(_, _) => unreachable!(),
        ...
        | Type::Forall(_, ty) => return layout_of(ty, target),
        | Type::Var(_) => unimplemented!(),
        | Type::Def(def) => unimplemented!(),
    };
...
pub fn ptr_sized_int(target: &Triple, sign: bool) -> Ty {
    match target.pointer_width() {
        | Ok(PointerWidth::U16) => Ty::new(Type::Int(16, sign)),
        | Ok(PointerWidth::U32) => Ty::new(Type::Int(32, sign)),
        | Ok(PointerWidth::U64) => Ty::new(Type::Int(64, sign)),
        | Err(_) => Ty::new(Type::Int(64, sign)),
    }
}
...
impl TyLayout {
    ...
    pub fn field(&self, field: usize, target: &Triple) -> Self {
        ...
        let ty = match &self.ty.kind {
            ...
            | Type::Forall(_, ty) => return layout_of(ty, target).field(field, target),
            | Type::Def(_) => unimplemented!(),
        };
...
impl Primitive {
    pub fn size(&self, triple: &target_lexicon::Triple) -> Size {
        match self {
            | Primitive::Bool => Size::from_bits(1),
    ...
    pub fn ty(&self) -> Ty {
        match self {
            ...
            | Primitive::Int(Integer::I8, sign) => Ty::new(Type::Int(8, *sign)),
            | Primitive::Int(Integer::I16, sign) => Ty::new(Type::Int(16, *sign)),
            | Primitive::Int(Integer::I32, sign) => Ty::new(Type::Int(32, *sign)),
            | Primitive::Int(Integer::I64, sign) => Ty::new(Type::Int(64, *sign)),
            | Primitive::Int(Integer::I128, sign) => Ty::new(Type::Int(128, *sign)),
            | Primitive::F32 => Ty::new(Type::Float(32)),
            | Primitive::F64 => Ty::new(Type::Float(64)),
